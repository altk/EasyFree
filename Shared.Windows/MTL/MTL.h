...
#pragma endregion
	...
	template <typename TDelegateInterface,
			  typename TCallback,
			  typename ... TArgs>
	inline Delegate<TDelegateInterface, TCallback, TArgs...> СreateDelegate(TCallback&& callback,
	...
	inline ComPtr<TDelegateInterface> CreateCallback(TCallback&& callback) NOEXCEPT
	{
		...
		auto delegate = СreateDelegate<TDelegateInterface>(std::forward<TCallback>(callback),
														   TTuple());
